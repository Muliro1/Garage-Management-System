...
function TechnicianLogin({setFullName,setPassword, handleLogin, error ,loading, props}) {
    return (
        <div className='technicianlogin h-[100vh]'>
            
           
            {loading ? <h1 className=' text-3xl text-white font-bold text-center pt-60'> Loading...</h1> :
                <>
            <div className='technicianlogobg'>
                <h1 className='technicianlogo text-white pl-4'>PBMW <span>Technician Login</span></h1>
                    </div>
            ...
            <div className=' rounded-lg bg-gradient-to-b from-slate-100 to-transparent via-grey-500 m-auto  w-5/12 mt-10'>
                ...
                <form className=' text-center mt-6 ' onSubmit={(e) => handleLogin(e, setFullName, setPassword, props)}>
                    ...
                    <input type='submit' value="LogIn" className='mt-6 mb-16 bg-black technicianloginbtn text-white p-1 rounded-2xl w-3/12 hover:cursor-pointer  hover:ease-in-out duration-300 hover:scale-125 hover:bg-white hover:text-black' />
                </form>
                <NavLink to='/' className='float-right mr-4 hover:text-orange-500'>Back to homepage</NavLink>
