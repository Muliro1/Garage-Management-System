...
function AdminClient({client}) {
    ...
    // console.log(client)
  return (
      <div className='pl-4 grid grid-cols-6 gap-6 bg-slate-300 rounded-lg pt-2 pb-1 mb-2 mr-2 ml-2 mt-6'>
          ...
          <p>{client.username}</p>
          <p>{client.email}</p>
          <p className=''>1234Nairobistreet</p>
          <p className=''>{client.telephone}</p>

          <svg className='ml-8' onClick={() => setIsOpen(!isOpen)} title="view client" width="22" height="18" viewBox="0 0 22 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          >
              <div >
                  <h1 className='text-center font-black underline italic'>Client's Information</h1>
                  <p>Clients Name:<span className='font-black'> {client.username}</span></p>
                  <p>Number of vehicles in garage:<span className='font-black'>{client.vehicles.length > 0?client.vehicles.length:'none'}</span></p>
                  <h1 className='font-black text-center mt-2 underline italic'>Vehicles</h1>
                  {client.vehicles.length > 0 && client.vehicles.map((vehicle) => (
                      <section key={vehicle.id} className=''>
                          <div className='bg-slate-200 mb-2 p-1 rounded-md'>
