...
function AdminGuards() {
    ...
    const [addGuard, setAddGuard] = useState(false)
    ...
    const getFilteredGuards = () => {
        if (!searchValue) return guards
        return guards.filter(
            guard => guard.full_name.toLowerCase().includes(searchValue.toLowerCase())
        )
    }
    ...
    return (
        <main className=''>
            ...
            <div className='flex mt-2'>
                ...
                <h1 className='pl-4 font-black tracking-wider'>Guards</h1>
            </div>
            <section className='bg-slate-300  pb-6 m-auto ml-8 mr-4 pt-2 '>
                <div className='flex ml-2 '>
                    <svg className='ml-2 ' width="22" height="22" viewBox="0 0 71 66" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <p onClick={() => setAddGuard(!addGuard)} className='pl-1 mt-2 font-bold text-white hover:text-orange-500 cursor-pointer'>Add a guard</p>
                </div>
                <div className={addGuard ? 'adminmechanictrue' : 'adminmechanicfalse'}>
                    <svg onClick={() => setAddGuard(!addGuard)} className='float-right mr-2 mt-2 hover:bg-orange-500' width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <form className='text-center' onSubmit={handleNewGuard}>
                        <p className='text-white pb-2 font-black text-lg pl-6'>Add new Guard</p>
                ...
                <div className='ml-4  flex justify-end'>
                    <div>
                        ...
                        <input onChange={(e) => setSearchValue(e.target.value)} className='rounded-3xl text-center w-7/12' type='search' placeholder='enter guard name....' />
                    </div>
                </div>
                <div className=' flex  '>
                ...
                <section className='bg-slate-200 m-auto h-[60vh] mt-2 w-11/12  pb-2 overflow-hidden hover:overflow-y-scroll rounded-md'>
                    ...
                    {filteredGuards.map((guard) => {
                        return (
                            <AdminGuard key={guard.id} guard={guard} refresh={refresh} setRefresh={setRefresh} />
                        )
                    })}
                    </div>
                </section>
